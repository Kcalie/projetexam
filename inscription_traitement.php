...
if(isset($_POST['submit'])){
    ...
    if(!empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['password2'])) {
        ...
        if($_POST['password'] == $_POST['password2']) {
            ...
            if($query->rowCount() == 0){
                /*chiffre le mot de passe soumis avec la fonction de hachage sha1().*/ 
                $password = sha1($_POST['password']);
                /*insère une nouvelle ligne dans la table "utilisateurs" avec l'adresse email et le mot de passe chiffré soumis dans le formulaire.*/
            ...
            /*Cette ligne redirige l'utilisateur vers la page d'accueil du site.*/
            header('location:index.php');
            }
            /*Si l'adresse email soumise dans le formulaire est déjà présente dans la table "utilisateurs", cette ligne affiche le message "Déjà enregistré".*/ 
            else{
